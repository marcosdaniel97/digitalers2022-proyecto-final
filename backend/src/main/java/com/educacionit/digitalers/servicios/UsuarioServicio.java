...
public class UsuarioServicio implements ServicioGenerico<Usuario, Integer> {
	...
	@Path("/buscarPorID{id}")
	@Produces(value = MediaType.APPLICATION_JSON)
	// @PathParam = buscarPorID/8 (es un recurso),
	// @QueryParam buscarPorID?id=8 (query paramter)
	public Response buscarPorID(@PathParam(value = "id") Integer id) {
		Usuario usuario = Usuario.builder().id(id).correo(id + "@gmail.com").clave("1234").activo(true).build();
	...
	@Path(value = "/listar")
	@Produces(value = MediaType.APPLICATION_JSON)
	public Response listar(Usuario v) {
